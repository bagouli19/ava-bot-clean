...
def charger_memoire_ava():
    ...
    try:
        ...
        print("⚠️ Erreur de chargement de la mémoire AVA.")
        return {"souvenirs": []}
    except Exception as e:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if contenu_base64:
                contenu_json = base64.b64decode(contenu_base64).decode("utf-8")
                return json.loads(contenu_json)
        print("⚠️ Erreur de chargement de la mémoire des utilisateurs.")
        return {}
    except Exception as e:
        print(f"⚠️ Erreur de connexion à GitHub : {e}")
        return {}
...
def sauvegarder_memoire_ava(memoire: dict):
    ...
    try:
        ...
        data = {"message": "🔄 Mise à jour de la mémoire AVA", "content": contenu_base64, "sha": sha}
...
def gerer_modules_speciaux(question: str, question_clean: str, model) -> Optional[str]:
    ...
    if any(kw in question_clean for kw in [
        ...
        "gencive douloureuse", "œdème", "sciatique", "gerçure aux mains", "trachéite", "kyste sébacé", "arthrite", 
        "hémorroïdes", "crise d’angoisse", "baisse de vue soudaine", "lésion cutanée", "spasmes musculaires", 
        "trouble digestif", "infection dentaire", "bruit de craquement dans les articulations", "tension basse", 
        "tension artérielle", "varicelle", "rougeole", "rubéole", "scarlatine", "otite externe", "otite moyenne", 
        "crise de goutte", "hallucinations", "pityriasis versicolor", "escarre", "syndrome du canal carpien", 
        "kyste pilonidal", "hallux rigidus", "hyperhidrose", "bégaiement", "torticolis congénital", 
        "spina bifida", "troubles obsessionnels compulsifs (TOC)", "déficit de l'attention", 
        "syndrome de Guillain-Barré", "intoxication alimentaire", "pied d'athlète", "zona ophtalmique", 
        "prostate", "lupus", "syndrome de Raynaud", "tuberculose", "incontinence urinaire", 
        "fracture de fatigue", "crise d'épilepsie", "pneumothorax", "syndrome de Ménière", 
        "polypes nasaux", "kératite", "mélanome", "paralysie faciale", "trichotillomanie", 
        "hyperthyroïdie", "hypothyroïdie", "rétention d'eau", "gluten", "anorexie mentale", 
        "boulimie", "pityriasis rosé", "herpès", "cicatrice", "brûlure thermique", 
        "érythème fessier", "colopathie fonctionnelle", "vertiges positionnels"
    ]):   
